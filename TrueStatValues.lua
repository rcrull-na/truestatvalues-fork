...
local statIdMap = {
    [CR_CRIT_SPELL]={["conversionFactor"]="CritConv",["rating"]="BaseCritRating",["tertiary"]=false},
    [CR_HASTE_SPELL]={["conversionFactor"]="HasteConv",["rating"]="BaseHasteRating",["tertiary"]=false},
    [CR_MASTERY]={["conversionFactor"]="MasteryConv",["rating"]="BaseMasteryRating",["tertiary"]=false},
    [CR_VERSATILITY_DAMAGE_DONE]={["conversionFactor"]="VersConv",["rating"]="BaseVersatilityRating",["tertiary"]=false},
    [CR_LIFESTEAL]={["conversionFactor"]="LeechConv",["rating"]="BaseLeechRating",["tertiary"]=true},
    [CR_AVOIDANCE]={["conversionFactor"]="AvoidConv",["rating"]="BaseAvoidanceRating",["tertiary"]=true},
    [CR_SPEED]={["conversionFactor"]="SpeedConv",["rating"]="BaseSpeedRating",["tertiary"]=true},
};
...
function addon.tsv:GetStatDiminishBracket(statId,amount)
    ...
    local true_rating = 0;
	local brackets;
	if stat.tertiary then
		brackets = self.TertiaryStatBrackets;
	else
		brackets = self.StatBrackets;
	end
    ...
    for i,bracket in ipairs(brackets) do
        if ( percent < bracket.size ) then
            ...
            bracket_next_penalty = brackets[i+1] and brackets[i+1].penalty or 1;
